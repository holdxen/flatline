...
impl<'a> FileReceiver<'a> {
    ...
    pub async fn receive(&mut self) -> error::Result<Vec<u8>> {
        ...
        if self.received == self.size + 1 {
            ...
            return Err(UnexpectedResponseSnafu {
                detail: format!("Unexpected file data: received={}, size={}", self.received, self.size)
            }.build().into());
        }
...
impl Handle {
    ...
    pub async fn start_receiving(&mut self, target: &str) -> error::Result<FileReceiver<'_>> {
        ...
        self.channel.channel_mut().request_exec(true, command).await?;
        ...
        let line = std::str::from_utf8(&line[..line.len() - 1]).context(UnexpectedErrorMessageSnafu)?;
        ...
        let mode = u16::from_str_radix(&caps[1], 8).ok().context(UnexpectedResponseSnafu {
            detail: format!("Unexpected response file mode: {}", &caps[1])
        })?;
        let size = caps[2].parse::<u64>().ok().context(UnexpectedResponseSnafu {
...
mod test {
    ...
    async fn test_sending_file() -> anyhow::Result<()> {
        ...
        let mut file_sender = stream.start_sending_file(0o655, file.len() as u64, name).await?;
