...
fn create<T: AsyncRead + AsyncWrite + Unpin + Send, N>(
    ...
    let inner = SessionInner::new(session_id, socket, notifier, signer, receiver, sender.downgrade());
...
impl AsRef<str> for InteractiveMethod {
    fn as_ref(&self) -> &str {
        match self {
            InteractiveMethod::PAM => {
                "pam"
            }
            InteractiveMethod::BSD => {
                "bsdauth"
            }
            InteractiveMethod::Other(method) => {
                method.as_str()
            }
        }
    }
...
pub trait KeyboardInteractive: Send + Sync{
    async fn interactive(&mut self, name: &str, instruction: &str, prompts: &[Prompt<'_>]) -> error::Result<Vec<String>>;
}
...
impl Session {
    ...
    pub async fn authenticate_public_key(&self,
                                         username: impl Into<String>,
        ...
        let event = if let Some(public_key_file) = public_key_file {
            ...
            match public {
                Public::Normal { r#type, content, comment } => {
                ...
                Public::Certificate { r#type, content, comment, principals, .. } => {
                    ...
                    if !principals.contains(&username) {
                        tracing::warn!("Maybe {} is not allowed to use this certificate to authenticate", username);
                    }
    ...
    pub async fn authenticate_keyboard_interactive(&self, username: impl Into<String>, interactive: Box<dyn KeyboardInteractive>, methods: Vec<InteractiveMethod>) -> error::Result<AuthenticateResult> {
        let (sender, receiver) = oneshot::channel();
    ...
    pub async fn handshake<T, N>(
        socket: T,
        config: Config,
        notifier: N,
    ) -> error::Result<Self> where
        ...
        N: Notifier + Send + 'static,
    {
        ...
        let (session, mut inner) = create(shaker.session_id.unwrap(), shaker.cipher_stream.take().unwrap(), shaker.notifier, shaker.config.signer);
    ...
    pub async fn listen_on_server(&self,
                                  addr: forward::SocketAddr,
                                  initial_window_size: u32,
                                  maximum_packet_size: u32) -> error::Result<forward::Listener> {
        let (sender, receiver) = oneshot::channel();
    ...
    pub async fn connect_to_server(&self, target: forward::SocketAddr, source: forward::SocketAddr, initial_window_size: u32, maximum_packet_size: u32) -> error::Result<forward::Stream> {
    ...
    pub async fn channel_open_default(&self) -> error::Result<Channel> {
        self.channel_open(Self::DEFAULT_INITIAL_WINDOW_SIZE, Self::DEFAULT_MAXIMUM_PACKET_SIZE).await
    }
    ...
    pub async fn sftp_open_default(&self) -> error::Result<sftp::Handle> {
        self.sftp_open(Self::DEFAULT_INITIAL_WINDOW_SIZE, Self::DEFAULT_MAXIMUM_PACKET_SIZE).await
    }

    pub async fn sftp_open(&self,
                           initial_window_size: u32,
                           maximum_packet_size: u32,
...
mod test {
    ...
    fn shuffle_indexmap<K, V, S>(map: &mut IndexMap<K, V, S>) {
    ...
    async fn test_authenticate_keyboard_interactive() {
        ...
        let session = Session::handshake(tcp, Config::default(), DefaultNotifier::default()).await.unwrap();
        ...
        impl KeyboardInteractive for KeyboardInteractiveImpl {
            async fn interactive(&mut self, name: &str, instruction: &str, prompts: &[Prompt<'_>]) -> error::Result<Vec<String>> {
                tracing::info!("Interactive: name={}, instruction={}, prompts={:?}", name, instruction, prompts);
                Ok(vec![PASSWORD.to_string(); prompts.len()])
            }
        }


        session.authenticate_keyboard_interactive(USERNAME, Box::new(KeyboardInteractiveImpl), Default::default()).await.unwrap();
    ...
    async fn test_authenticate_public_key() {
        ...
        if !result.success() {
            ...
            result = session.authenticate_public_key(USERNAME, &private_key_file, Some(&public_key_file), None).await.unwrap();
        }
    ...
    async fn test_handshake() {
        ...
        for _ in 0..999 {
            ...
            shuffle_indexmap(&mut config.kex);
            shuffle_indexmap(&mut config.host_key);
            shuffle_indexmap(&mut config.crypt_client_to_server);
            shuffle_indexmap(&mut config.crypt_server_to_client);
            shuffle_indexmap(&mut config.mac_client_to_server);
            shuffle_indexmap(&mut config.mac_server_to_client);
            shuffle_indexmap(&mut config.compress_client_to_server);
            shuffle_indexmap(&mut config.compress_server_to_client);
            ...
            tracing::info!(
                "encode: {:?}",
                config.compress_client_to_server.keys()
            );
            tracing::info!(
                "decode: {:?}",
                config.compress_server_to_client.keys()
            );
            let tcp = tokio::net::TcpStream::connect((HOST, PORT)).await.unwrap();
            let session = Session::handshake(tcp, config, DefaultNotifier::default()).await.unwrap();
            session.request_authentication().await.unwrap();
            let mut result = session.authenticate_none(USERNAME).await.unwrap();
            if !result.success() {
                result = session
